...
std::vector<int> InformationManager::getGroupForce(std::vector<UnitInfo> & unitInfos)
{
	...
	{
		groundPower += unitinfo.groudWeapon;
		airPower += unitinfo.airWeapon;
		healthPower += unitinfo.lastHealth;
	}

	forces.push_back(groundPower);
	forces.push_back(airPower);
	forces.push_back(healthPower);
